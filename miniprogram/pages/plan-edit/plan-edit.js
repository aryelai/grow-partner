...
Page({
  ...
  async save() { if (this.saveInProgress) return; this.saveInProgress = true; this.setData({ submitting: true }); try { if (!await this.refreshPermission()) return; const items = this.data.form.items.map(({ text, isDone, priority }) => ({ text, isDone, priority })).filter((item) => item.text.trim()); if (!this.data.form.title.trim() || !items.length) { wx.showToast({ title: "请填写标题和至少一个子任务", icon: "none" }); return; } await callFunction("plan", this.data.id ? "update" : "create", { ...this.data.form, id: this.data.id, items }); wx.navigateBack(); } catch (error) { showError(error); } finally { this.saveInProgress = false; this.setData({ submitting: false }); } },
  async remove() { if (!await this.refreshPermission()) return; wx.showModal({ title: "删除计划", content: "删除后无法恢复，确认继续吗？", success: async (result) => { if (!result.confirm) return; if (!await this.refreshPermission()) return; try { await callFunction("plan", "remove", { id: this.data.id }); wx.navigateBack(); } catch (error) { showError(error); } } }); },
});
