...
- 覆盖 1–3 张、4 MB 边界、非法 MIME、代码块 JSON、未知科目、模糊截止时间、超长字段和最多 60 条。
...
- 修改 `cloudfunctions/ai/core.js`、`cloudfunctions/ai/index.js`
- 修改 `miniprogram/utils/ai-import.js`、`miniprogram/pages/homework-import/*`
- 修改 `tests/ai-import-core.test.js`、`tests/ai-import-service.test.js`、`tests/ai-import-client.test.js`

**工作：**

- 增加 `auto`、工作日、星期五/周末和整周范围；创建任务时服务端白名单校验并绑定，分析时只读任务范围。
- `auto` 对周表取最右侧有效列，对单日清单、聊天记录和普通单日截图取全部；手动星期仅约束周表，整周取全部非空列。
- 清洗并展示版式、服务端派生的识别范围、周次和不确定字段；接收上限提升到 60 条，并根据模型 `truncated`、CloudBase `finish_reason` 和完成 token 数提示按星期分批识别。
- 使用 `created_at_desc` 白名单排序单次读取当前学期最近 50 条已有作业，避免重复调用作业列表造成数据库扫描放大；数据库中或同批次的精确重复项默认取消勾选但允许重新勾选，检查失败不阻断。
- 保持临时文件、权限、额度、上传凭据、清理和幂等保存边界不变；不部署、不推送。
