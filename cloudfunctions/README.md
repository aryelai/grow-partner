...
- [x] `login`、`family` 均配置 `REGISTRATION_MODE=family_invite` 并部署到 `cloud1`；2026-09-12 核对 `$LATEST` 更新时间分别为 15:39:11、15:39:41，环境变量值均为 `family_invite`。
- [ ] 无邀请码注册、错误邀请码、有效邀请码、既有账号和新建家庭拒绝均完成真机验证。
- [ ] 云存储使用自定义安全规则，`read` 仅允许原上传者，`write` 为 `false`；规则生效后验证已有文件可读且客户端上传被拒绝。
  - 2026-09-12 保存时控制台明确提示免费环境无法使用自定义安全规则，原有“仅创建者可读写”规则未改动。可选的“所有用户不可读写”会同时破坏历史文件读取，未在没有额外业务确认时采用。
...
- 2026-09-11 17:19 已重新部署包含等待队列轮转修复的 `reminder`；控制台显示状态为“已部署”，运行环境仍为 Node.js 16.13，执行方法仍为 `index.main`，内存 256 MB、超时 3 秒，`MINIPROGRAM_STATE` 仍为 `developer`。
- 无可用订阅次数的候选任务现在会把下一次尝试时间后移 1 分钟，避免固定的前 50 条任务永久占住全局扫描窗口；自动化测试已覆盖 50 条无额度任务之后的第 51 条可发送任务能够在下一轮被选中。
- 当前轮转方案仍有明确容量边界：按每轮最多处理 50 条、每 30 分钟一轮计算，同一两小时有效窗口的理论上限约为 200 条更早的无额度任务；3 秒云函数超时可能让实际容量更低。受控的小规模家庭内测可在接受该限制后使用，公开扩量前必须改为每个接收人唯一活跃槽的队列模型并完成旧数据迁移。
...
- 用户已确认手机收到“开发环境提醒联调 2026-09-11”，因此订阅授权、自然调度和实际送达已经贯通。
- 手机上的消息点击未进入只读详情页；从消息下方“编辑通知”进入后标题和来源为空。当前本地代码的发送参数已固定为 `pages/notice-detail/notice-detail?id=<通知ID>`，开发者工具使用同一条真实通知验证了只读详情和编辑页均能正确显示标题、来源，自动化测试也锁定了页面路径、通知 ID 传递和字段保留。
- 结合“本轮只部署过云函数、尚未上传最新小程序代码包”的事实，当前最高概率原因是手机打开了此前上传的旧开发版代码。该判断仍需在上传最新 `1.0.0` 后重新发送一条真实消息验证；复测通过前不得宣称消息落地闭环完成。
